...
Inverse relationship, the less the miles per gallon, the more the cylinder. 
---
...
'hp_per_wt' is useful because cars have different weight, so the metric shows power relative to size. Two cars with the same horsepower can feel very different to drive if one is much heavier.
'qsec_category' is useful because it translates raw quarter-mile times into simple categories (Fast, Medium, Slow), making it easier to compare performance across many cars without needing to interpret exact times.
---
...
Cars with fewer cylinders generally have the highest average mpg, while 8-cylinder cars show the lowest fuel efficiency. Within the same cylinder group, cars with manual transmission (am = 1) often show higher average mpg than their automatic counterparts.Median horsepower tends to rise with the number of cylinders, so 8-cylinder cars are the most powerful, while 4-cylinder cars are the least.
...
In the wide format, each growth rate must be calculated with a separate formula, which quickly becomes repetitive and unmanageable with many years of data. In contrast, the long format allows a single mutate() with lag() to handle all years at once, making the code much more concise and scalable. The long format is generally preferred for time-series calculations because it follows tidy data principles and makes it easier to apply functions consistently across time.
---
...
The inner_join() only includes students with both demographics and grades, excluding students like Frank (S007) who have no grades. The left_join() keeps all students from the demographics table, so Frank appears with NA for grade-related fields, while S006, who lacks demographics, does not appear in either join. The average grade calculation summarizes multiple grades for students like Alice and clearly shows NA for students with no grades.
---
...
The semi_join identifies students who have both demographic information and grade records, confirming active enrollment. The anti_join on demographics highlights students without any grades, such as new enrollees or those who haven’t completed courses. The anti_join on grades reveals grade records with no matching student, indicating potential data errors. Together, these joins help validate the dataset and assess the completeness and accuracy of student records.
