...
# Your code here
mtcars_filtered <- mtcars_processed %>%
...
per gallon in this subset? Inverse relationship, the less the miles per
gallon, the more the cylinder. —
...
‘hp_per_wt’ is useful because cars have different weight, so the metric
shows power relative to size. Two cars with the same horsepower can feel
very different to drive if one is much heavier. ‘qsec_category’ is
useful because it translates raw quarter-mile times into simple
categories (Fast, Medium, Slow), making it easier to compare performance
across many cars without needing to interpret exact times. —
...
mtcars_summary <- mtcars_processed %>%
  group_by(cyl, am) %>%
  summarise(average_mpg = mean(mpg),median_hp = median(hp),num_cars = n(),.groups = "drop") %>%
...
differentiate car characteristics? Cars with fewer cylinders generally
have the highest average mpg, while 8-cylinder cars show the lowest fuel
efficiency. Within the same cylinder group, cars with manual
transmission (am = 1) often show higher average mpg than their automatic
counterparts.Median horsepower tends to rise with the number of
cylinders, so 8-cylinder cars are the most powerful, while 4-cylinder
cars are the least.
...
product_sales_long <- product_sales_wide %>%
  pivot_longer(cols = starts_with("Year_"), names_to = "Year", values_to = "Sales") %>%
 mutate(Year = as.numeric(str_replace(Year, "Year_", "")))
product_sales_long
```

    ## # A tibble: 9 × 5
    ##   product  Growth_2021 Growth_2022  Year Sales
    ##   <chr>          <dbl>       <dbl> <dbl> <dbl>
    ## 1 Laptop         10          13.6   2020  1000
    ## 2 Laptop         10          13.6   2021  1100
    ## 3 Laptop         10          13.6   2022  1250
    ## 4 Monitor        10           9.09  2020   500
    ## 5 Monitor        10           9.09  2021   550
    ## 6 Monitor        10           9.09  2022   600
    ## 7 Keyboard        6.25        5.88  2020   800
    ## 8 Keyboard        6.25        5.88  2021   850
    ## 9 Keyboard        6.25        5.88  2022   900
...
```
    ...
    ## # A tibble: 9 × 6
    ##   product  Growth_2021 Growth_2022  Year Sales Growth_Rate
    ##   <chr>          <dbl>       <dbl> <dbl> <dbl>       <dbl>
    ## 1 Keyboard        6.25        5.88  2020   800       NA   
    ## 2 Keyboard        6.25        5.88  2021   850        6.25
    ## 3 Keyboard        6.25        5.88  2022   900        5.88
    ## 4 Laptop         10          13.6   2020  1000       NA   
    ## 5 Laptop         10          13.6   2021  1100       10   
    ## 6 Laptop         10          13.6   2022  1250       13.6 
    ## 7 Monitor        10           9.09  2020   500       NA   
    ## 8 Monitor        10           9.09  2021   550       10   
    ## 9 Monitor        10           9.09  2022   600        9.09
...
generally preferred for this type of time-series calculation? In the
wide format, each growth rate must be calculated with a separate
formula, which quickly becomes repetitive and unmanageable with many
years of data. In contrast, the long format allows a single mutate()
with lag() to handle all years at once, making the code much more
concise and scalable. The long format is generally preferred for
time-series calculations because it follows tidy data principles and
makes it easier to apply functions consistently across time. —
...
inner_join_result <- students_demographics %>%
...
left_join_result <- students_demographics %>%
...
avg_grade_result <- left_join_result %>%
...
no grades? The inner_join() only includes students with both
demographics and grades, excluding students like Frank (S007) who have
no grades. The left_join() keeps all students from the demographics
table, so Frank appears with NA for grade-related fields, while S006,
who lacks demographics, does not appear in either join. The average
grade calculation summarizes multiple grades for students like Alice and
clearly shows NA for students with no grades. —
...
understanding the completeness of your student records? The semi_join
identifies students who have both demographic information and grade
records, confirming active enrollment. The anti_join on demographics
highlights students without any grades, such as new enrollees or those
who haven’t completed courses. The anti_join on grades reveals grade
records with no matching student, indicating potential data errors.
Together, these joins help validate the dataset and assess the
completeness and accuracy of student records.
